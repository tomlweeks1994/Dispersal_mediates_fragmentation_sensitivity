...
## create two subsets based on tree cover change
...
model_expanded <- brm(
  ...
  file = paste0("Output/large_TC_change_model_expanded")
)

posterior_samples <- posterior_samples(model_expanded)
saveRDS(posterior_samples, "Output/full_medium_model_full_posterior_samples_largest_change.rds")