...
## create subsets based on interim tree cover change
...
model_restricted <- brm(
  ...
  file = paste0("Output/large_TC_change_model_restricted")
)

posterior_samples <- posterior_samples(model_restricted)
saveRDS(posterior_samples, "Output/full_high_model_full_posterior_samples_largest_change.rds")