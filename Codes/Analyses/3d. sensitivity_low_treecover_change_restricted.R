...
## subset the data based on interim tree cover change
...
model_restricted <- brm(
  ...
  file = paste0("Output/small_TC_change_model_restricted")
)

posterior_samples <- posterior_samples(model_expanded)
saveRDS(posterior_samples, "Output/full_high_model_full_posterior_samples_smallest_change.rds")