...
## subset based on interim tree cover change
...
## reduce dataset
phylo_data <- phylo_data %>% filter(Dataset_ID %in% smallest_change_dataset_IDs)
...
#Read in all available Hackett back trees available at bridtree.org
...
model_expanded <- brm(
  ...
  file = paste0("Output/small_TC_change_model_expanded")
)

posterior_samples <- posterior_samples(model_expanded)
saveRDS(posterior_samples, "Output/full_medium_model_full_posterior_samples_smallest_change.rds")