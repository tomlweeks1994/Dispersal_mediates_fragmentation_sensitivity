...
library(wesanderson)
library(hier.part)
source("../../Custom functions/scale_2sd.R")
...
data_reduced$nHWI_z <- scale_2sd(data_reduced$nHWI)
data_reduced$Disturbance_mean <- scale_2sd(data_reduced$Disturbance_mean)
data_reduced$Seasonality_mean <- scale_2sd(data_reduced$Seasonality_mean)
data_reduced$Abs_latitude_mean <- scale_2sd(data_reduced$Abs_latitude_mean)
...
# Drop the duplicates - keeping only the first one #
rand_data_2 <- as.data.frame(s %>% distinct(Jetz_name, .keep_all = TRUE))
...
## plot forest plot ##
main_full <- ggplot(data=df_all, aes(x=Coefficient, y=Estimate))+ #excluding intercept because estimates so much larger
  ...
  # geom_pointrange(aes(ymin=Estimate-CI_interval, ymax=Estimate+CI_interval,
  #                     color = Coefficient, alpha = Sample), fatten = 9, lwd = 1/2, shape = 21,
  #                 fill = "white", position = pd, key_glyph = draw_key_pointrange, show.legend=F) +
  # 
...
pd <- position_dodge(0.75)
## plot forest plot ##
main_dist_lat <- ggplot(data=df_all, aes(x=Coefficient, y=Estimate))+ #excluding intercept because estimates so much larger
  theme_classic() + #clean graph
  geom_hline(yintercept = 0, lty = 2) +
  geom_linerange(aes(ymin=Estimate-CI_interval, ymax=Estimate+CI_interval, 
                     colour = Coefficient, alpha = Sample),lwd = 3, position = pd, show.legend=F) + 
  ...
  #geom_vline(xintercept=-1, show.legend=T)+
...
pd <- position_dodge(0.75)
## plot forest plot ##
main_dist <- ggplot(data=df_all, aes(x=Coefficient, y=Estimate))+ #excluding intercept because estimates so much larger
  theme_classic() + #clean graph
...
#grid.arrange(arrow, main_dist, main_dist_lat, main_full, ncol = 1)
...
png("Figures/Publication/Extended Data/ED - 4 nHWI drivers (distributional)_new.png",
...
## Legend is edited outside of R

tiff("Figures/Publication/Extended Data/ED - 4 nHWI drivers (distributional)_tiff.tif",
    width = 900, height = 650)
