...
plot <- ggplot() +
  ...
  theme(legend.position = "top",
        ...
        plot.title = element_text(size = 30, face = "bold", hjust = 0.08, vjust = -10)) 

pmain <-  plot  +
  annotate("text", x= 0.7, y= -1.15, label= paste0("Coefficient: ", round(as.numeric(mod$coefficients[2]), 3)), size =7) +
  annotate("text", x= 0.7125, y= -1.3, label= paste0("P-value < 0.001"), size = 7)
...
whisker <- ggplot(data) + 
  ...
  #geom_dotplot(aes(x = Disturbance, y = abs_lat, fill = Disturbance),binaxis = "y", binwidth = 0.5, stackdir = "center") +
  #stat_summary(aes(x = Disturbance, y = abs_lat), fun = median, geom = "crossbar") +
  theme_bw() +
  ggtitle("a") +
