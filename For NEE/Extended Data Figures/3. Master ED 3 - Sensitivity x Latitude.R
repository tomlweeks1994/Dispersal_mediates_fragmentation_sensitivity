...
mod <- glm(prop_sensitive ~ abs_lat, data=data1, family = "quasipoisson")
summary(mod)
pred = NULL
n_pred = 100
...
pmain_expanded <-  pmain_expanded + geom_point(data = data1,  aes(y = prop_sensitive, x = abs_lat,  color = Disturbed_factor), size = 7) +
  ...
  annotate("text", x=52.8, y=0.63, label= paste0("P-value: ", round(summary(mod)$coefficients[2,4], 3)), size = 15)
...
#########
form = as.formula("prop_sensitive ~ abs_lat")
mod <- glm(prop_sensitive ~ abs_lat, data=data2, family = "quasipoisson")
summary(mod)
pred = NULL
n_pred = 100
...
pmain_restricted <-  pmain_restricted + geom_point(data = data2,  aes(y = prop_sensitive, x = abs_lat,  color = Disturbed_factor), size = 7) +
  scale_color_manual(values = c("red", "deepskyblue")) +
  annotate("text", x=50, y=0.35, label= paste0("Coefficient: ", round(as.numeric(mod$coefficients[2]), 3)), size = 15) +
  annotate("text", x=52.8, y=0.32, label= paste0("P-value: ", round(summary(mod)$coefficients[2,4], 3)), size = 15) + 
