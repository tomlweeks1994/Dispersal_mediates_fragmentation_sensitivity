...
species_data <- read_excel("NEE main files/First Review/Supplementatry Dataset 1_oct20.xlsx", sheet = 3) %>%
  ...
  dplyr::filter(Order != "Apodiformes") ## Sensitivity ana
...
#########################################CREATE THE DATA FOR PASSERINES MODEL#############################################
#subset the dataset for passerine species only
passerine_data <- phylo_data %>% filter(Order == "Passeriformes")

#create a vector of expected tree tips
pass_Tips <- passerine_data$Jetz_name

#Create empty lists for brms to run through
pass_phylo_list <- list()
...
for (i in 1:50) {
  ...
  my_tree_i <- ape::drop.tip(my_tree_i, setdiff(my_tree_i$tip.label, pass_Tips)) #droptips in the tree so it is the same as my data
  
  Ap_i <-ape::vcv.phylo(my_tree_i) #convert to vcv matrix (brms format for phylogeny)
  
  pass_phylo_list[[i]] <- list(A = Ap_i) #populate
  
}

#############################################CREATE THE DATA FOR NONPASS MODELS#############################################
#subset the dataset for non-passerine species
nonpasserine_data <- phylo_data %>% filter(Order != "Passeriformes") 

#Create a vector of expected tree tips
nonpass_Tips <- nonpasserine_data$Jetz_name

#Create empty lists for brms to run through
nonpass_phylo_list <- list()
...
for (i in 1:50) {
  ...
  my_tree_i <- ape::drop.tip(my_tree_i, setdiff(my_tree_i$tip.label, nonpass_Tips)) #droptips in the tree so it is the same as my data
  
  Apn_i <-ape::vcv.phylo(my_tree_i) #convert to vcv matrix (brms format for phylogeny)
  
  nonpass_phylo_list[[i]] <- list(A = Apn_i) #populate
...
full_model_medium <- brm(
  dep_sense_medium ~ nHWI_z * Mass_log_z + Disturbance + abs_lat_z  + Seasonality_z + (1 | Dataset_ID) + (1 | Species_name) + (1 | gr(Jetz_name, cov = A)),
  data = phylo_data,
  family = bernoulli(),
  ...
  file = paste0("../output/main_models_nApod/full_model_medium", array_number)
)
