...
#Read in the data
site_data <- read_excel("NEE main files/First Review/Supplementatry Dataset 1_sept22.xlsx", sheet = 2) %>%
  dplyr::select("Dataset ID", "Latitude", "Disturbance")

species_data <- read_excel("NEE main files/First Review/Supplementatry Dataset 1_sept22.xlsx", sheet = 3) %>%
  dplyr::select("Species name", "Order", "nHWI", "Mass (g)", "Forest_dependency")

site_sp_list <- read_excel("NEE main files/First Review/Supplementatry Dataset 1_sept22.xlsx", sheet = 4)
...
full_model_medium <- brm_multiple(
  dep_sense_medium ~ nHWI_z * Mass_log_z + Disturbance + abs_lat_z + (1 | `Dataset ID`) + (1 | `Species name`) + (1 | gr(Jetz_name, cov = A)), 
  data = my_data_list,
  family = bernoulli(),
  data2 = my_phylo_list,
  warmup = 2000,
  ...
  file = "full_model_medium"
)


### high
full_model_high <- brm_multiple(
  dep_sense ~  nHWI_z * Mass_log_z + Disturbance + abs_lat_z + (1 | `Dataset ID`) + (1 | `Species name`) + (1 | gr(Jetz_name, cov = A)), 
  family = bernoulli(),
  data2 = my_phylo_list,
  warmup = 2000,
  iter = 5000,
